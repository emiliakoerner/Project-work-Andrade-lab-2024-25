...
print("Total entries in gtf_df:", gtf_df.shape[0])          #check numbers of genes and transcripts
...
##################################################################################################################
referenceproteome = 'reference_proteome.fasta'  # reference proteome of 20.656 proteins
# Code to store protein data from proteome file in a dictionary (UniprotID, gene name and sequence)
...
with open(mapping_ensembl, 'r') as map:
    for map_line in map:
...
with open(housekeeping_file, 'r') as hk_file:
...
print("unique gene names in housekeeping file:",len(housekeeping_genes))
...
mapped = 'proteome_mapped_hk_genename.txt'
with open(mapped, 'w') as mapped:
    ...
    for uniprot_id, gene_name, length, hk_status in no_gn:         # add all proteins without gene name to the file
